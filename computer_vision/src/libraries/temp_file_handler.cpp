...
*/
bool TempFileHandler::writeMatBinary(std::ofstream& ofs, const cv::Mat& out_mat) {
	if(!ofs.is_open()){
		return false;
	}
	if(out_mat.empty()){
...
*/
bool TempFileHandler::SaveMatBinary(const std::string& filename, const cv::Mat& output) {
...
*/
bool TempFileHandler::readMatBinary(std::ifstream& ifs, cv::Mat& in_mat) {
	if(!ifs.is_open()){
		return false;
	}
...
*/
bool TempFileHandler::LoadMatBinary(const std::string& filename, cv::Mat& output) {
