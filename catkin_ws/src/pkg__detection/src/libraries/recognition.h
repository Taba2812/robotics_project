...
namespace recognition {
    ...
    std::vector<Types::RecognitionResult> runRecognition(cv::InputOutputArray img);
    ...
    std::vector<Types::RecognitionResult> detection(cv::Ptr<cv::GeneralizedHoughGuil> guil, std::list<cv::Mat> *buffer, cv::InputOutputArray img, std::vector<cv::Vec4f> position);

    void drawResults(cv::InputOutputArray img, std::vector<cv::Vec4f> position, std::list<cv::Mat> *buffer, std::vector<cv::Mat> pTemplate);
    void drawSelected(cv::Mat img, Types::RecognitionResult selection);
